/**
 * Rate limiter — bounds the expensive POSTs (LLM generation) at the Nitro BFF
 * choke point. A sliding-window counter keyed on client IP and session id. When
 * a key exceeds its budget within the window, the request is rejected with 429.
 *
 * In-memory and per-instance: the counters live in a closure `Map`, so they are
 * lost on scale-to-zero and NOT shared across multiple Nitro instances. This is
 * a best-effort bound for short bursts, not a hard, durable, global limit.
...
export function createRateLimiter(
    ...
    function windowOf(map: Map<string, Bucket>, key: string, max: number): LimitWindow {
        const now = clock.now();
        const existing = map.get(key);
        if (!existing || now >= existing.resetAt) {
