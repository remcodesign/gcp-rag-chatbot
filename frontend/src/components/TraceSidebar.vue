    ...
    <aside
        ...
        <div class="flex flex-none items-center justify-between border-b border-(--border) px-4 py-3.5">
            ...
            <button
                ...
                @click="emit('close')" aria-label="Close RAG trace">×</button>
        </div>
        <div class="no-scrollbar min-h-0 flex-1 overflow-y-auto p-4">
            <div class="mb-4 rounded-lg border border-(--border) px-3 py-2.5 text-[12px]"
            ...
            <template v-else>
                ...
                <div v-if="hasUsage" class="mb-6">
                    ...
                    <div class="mt-2 flex flex-col gap-1.5">
                        <div v-if="trace.usage?.promptTokens != null" class="flex items-center gap-2 text-[11px]">
                            ...
                            <span class="text-[11px] font-semibold text-(--text)">{{ trace.usage.promptTokens }}
                                tok</span>
                        </div>
                        <div v-if="trace.usage?.completionTokens != null" class="flex items-center gap-2 text-[11px]">
                            ...
                            <span class="text-[11px] font-semibold text-(--text)">{{ trace.usage.completionTokens }}
                                tok</span>
                        </div>
                        <div v-if="trace.usage?.totalTokens != null" class="flex items-center gap-2 text-[11px]">
                            ...
                            <span class="text-[11px] font-semibold text-(--text)">{{ trace.usage.totalTokens }}
                                tok</span>
                        </div>
                        ...
                        <div class="flex items-center gap-2 text-[11px]">
                            ...
                            <span class="text-[11px] font-semibold text-(--text)">{{
                                formatTokensPerSecond(trace.tokensPerSecond) }}</span>
                        </div>
                        <div v-if="trace.usage?.cost != null" class="flex items-center gap-2 text-[11px]">
                            <span class="w-14 shrink-0 text-(--muted)">Cost</span>
                            <span class="text-[11px] font-semibold text-(--text)">{{ formatCost(trace.usage.cost)
                                }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-6">
                    <h3 class="m-0 mb-1.5 text-[12px] uppercase tracking-[0.05em] text-(--muted)">Retrieval ({{
                        trace.retrieved.length }})</h3>
                    <ul class="m-0 flex list-none flex-col gap-2 p-0">
                        <li v-for="c in trace.retrieved" :key="c.id"
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <button
                                    ...
                                    type="button" @click="emit('open-chunk', c)">{{ c.title }}</button>
                            ...
                            <p class="m-0 mt-1 text-[12px] leading-[1.45] whitespace-pre-wrap text-[#3b4757]">{{
                                c.textPreview }}</p>
                        </li>
                    </ul>
                </div>
                ...
                <div class="mb-6">
                    <h3 class="m-0 mb-1.5 text-[12px] uppercase tracking-[0.05em] text-(--muted)">Context passed to LLM
                        ({{ trace.context.sources.length }})</h3>
                ...
                <div v-if="trace.finalPrompt" class="mb-4">
                    ...
                    <pre
                        class="no-scrollbar m-0 max-h-120 overflow-y-auto whitespace-pre-wrap rounded-lg border border-(--border) bg-(--bg) px-2.5 py-2.5 text-[11px] text-(--text)">{{ trace.finalPrompt }}</pre>
                </div>
            </template>
        </div>
    </aside>
